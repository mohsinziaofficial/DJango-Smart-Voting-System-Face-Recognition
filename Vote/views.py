    ...
    if request.method == 'POST':
        ...
        if user_login.user is not None:
            if user_login.user.is_active:
                # comment the next line if you don't want user to login without face recognition
...
def home(request):
    context = {}
    return render(request, 'Vote/home.html', {'nbar': 'home'})
...
def results(request):
    ...
    for c in pos:
        ...
        plt.axis('equal')
        name = "E:\\Python Projects\\Smart-Voting-System\\Vote\\static\\Vote\\" + c.position + '.png'
        fig.savefig(name)
        can[0].append(c)
        can[0].append(name)
        plt.close(fig)
...
def about(request):
    context = {}
    return render(request, 'Vote/about.html', {'nbar': 'about'})
...
def create_dataset(request):
    ...
    faceDetect = cv2.CascadeClassifier('E:\\Python Projects\\Smart-Voting-System\\ml\\haarcascade_frontalface_default.xml')
    ...
    while(True):
        ...
        for(x,y,w,h) in faces:
            ...
            cv2.imwrite('E:\\Python Projects\\Smart-Voting-System\\ml\\dataset\\'+str(id)+'.'+str(sampleNum)+'.jpg', gray[y:y+h,x:x+w])
...
def trainer(request):
    ...
    #Path of the samples
    path = 'E:\\Python Projects\\Smart-Voting-System\\ml\\dataset'
    ...
    recognizer.save('E:\\Python Projects\\Smart-Voting-System\\ml\\recognizer\\trainingData.yml')
    cv2.destroyAllWindows()
...
def detect(request):
    ...
    rec.read('E:\\Python Projects\\Smart-Voting-System\\ml\\recognizer\\trainingData.yml')
    getId = 0
...
def face_index(request):
    return render(request, 'face_index.html', {'nbar': 'face_index'})
